...
enum NetworkError: Error, Equatable, Sendable {
   static func == (lhs: NetworkError, rhs: NetworkError) -> Bool {
      ...
      case (.decodingKeyNotFoundFailure(let lEntity, let lKey), .decodingKeyNotFoundFailure(let rEntity, let rKey)): lEntity == rEntity && lKey == rKey
      case (.decodingValueNotFoundFailure(let lEntity, let lKey, let lValue), .decodingValueNotFoundFailure(let rEntity, let rKey, let rValue)): lEntity == rEntity && lKey == rKey && lValue == rValue
      case (.decodingTypeMismatchFailure(let lEntity, let lKey, let lType), .decodingTypeMismatchFailure(let rEntity, let rKey, let rType)): lEntity == rEntity && lKey == rKey && lType == rType
      default: false
      }
   }
